...
float WINDUP = 0.16622;
float PING = 28.0;

Orbwalker::Orbwalker(LoLProcessInfo& lolInfo)
    : lolInfo(lolInfo), heroListArray(ReadHeroListArray(lolInfo)), heroListSize(0),
...
{
    return (1 / localHero.ReadHeroAttackSpeed() * 1000) * windupPercentage;
}
        ...
        if (IsSpacebarHeldDown()) {
            if (CanAttack()) {
                ...
                if (target != nullptr) {
                    ...
                    lastMove = GetTickCount() + GetWindupTime();
                }
            }
            if (CanMove()) {
                SimulateRightClick();
